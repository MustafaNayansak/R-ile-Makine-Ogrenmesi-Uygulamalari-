...
deneme.veri.evet <- basari[sample(nrow(basari),73),]  #Satýr sayýsý az olan deðerin yarýsý modele sokulur
deneme.veri.hayir <- basarisizlik[sample(nrow(basarisizlik),73/x),]  #Baþarý ve Baþarýsýzlýk arasýndaki kat oranýný korumak için X ile çarpýlýr
model.veri <- rbind(deneme.veri.evet,deneme.veri.hayir)
nrow(model.veri) 

tahmin.veri.evet <- basari[sample(nrow(basari),73),]  #Tahmin verisinin baþarý sayýsý
tahmin.veri.hayir <- basarisizlik[sample(nrow(basarisizlik), 73/x),]   #Baþarýsýzlýk sayýsý
...
# Model oluþturduktan sonra tahmin verisi ile modeli test ediyoruz
model.tahmin <- predict(model3,tahmin.veri)
model.tahmin

# True Positive ile False Positive Rate ler ile bir ROC eðrisi çizdirilir
